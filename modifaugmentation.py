...
#    x=x[:, offsetw:offsetw+crop_size, offseth:offseth+crop_size]
#    return x
def color_augment(x):
	correction_vector=sio.loadmat('TrainIlluminant.mat');
	div=correction_vector.get('div');
#	f=h5py.File('SelectedIlluminant.mat','r');
#	correction_vector=f.get('div');
	ind=randint(0,605)
	cv=div[ind,:]
#	if K.backend()=='tensorflow':
#		for i in range(3):
#			x[:,:,i]=x[:,:,i]*(cv[i]*np.power(3,.5))
#	else:
	for i in range(3):
		x[i,:,:]=x[i,:,:]*(cv[i]*np.power(3,.5))	
	return x
